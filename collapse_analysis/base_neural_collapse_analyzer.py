...
class BaseNeuralCollapseAnalyzer:
    ...
    def test_nc1_variability_collapse(self):
        ...
        We measure this by the trace of the within-class covariance matrix (Tr(Sw)).
        A smaller value indicates stronger collapse.
        """
        if self.Sw is None: self._compute_class_means_and_covariances()
        
        # The "collapse" is reflected by a small trace of Sw relative to Sb
        ...
        print("Interpretation: A smaller ratio indicates that features of the same class are tightly clustered compared to how far apart the different class clusters are.")
    ...
    def test_nc2_etf_class_means(self):
        ...
        This means ||μ_c|| is constant and the angle between any two μ_i, μ_j is the same.
        """
        ...
        # Calculate the deviation from the ideal ETF structure
        ...
        print("Interpretation: A value close to 0 indicates the class means have formed a geometry similar to a perfect simplex ETF.")
    ...
    def test_nc3_self_dual_alignment(self):
        ...
        # 1. Check if classifier weights form an ETF
        ...
        # 2. Check alignment between W and μ
        ...
        # (W_norm.T @ mu_norm).diag() would be faster if matrices were small
        # but this is more explicit for each pair.
        ...
        print("Interpretation: A small deviation and a cosine similarity close to 1 indicate that the classifier's decision boundaries are aligned with the collapsed feature geometry.")
    ...
    def test_nc4_nearest_class_mean(self):
        ...
        model_preds = torch.argmax(self.features @ self.model.classifier.weight.T.cpu() + self.model.classifier.bias.cpu(), dim=1)
        
        # Get nearest clss-mean predictions using squared Euclidean distance = argmin
        ...
        print("Interpretation: An agreement close to 1.0 means the network's final layer is effectively just finding the closest class mean in the feature space.")
